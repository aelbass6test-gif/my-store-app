...
const CustomersPage: React.FC<CustomersPageProps> = ({ orders, customers: persistentCustomers, setCustomers, loyaltyData, updateCustomerLoyaltyPoints }) => {
  ...
  const customers = useMemo(() => {
    ...
    // Initial load from persistent customers
    persistentCustomers.forEach(c => {
        const cleanPhone = (c.phone || '').replace(/\s/g, '').replace('+2', '');
        if (cleanPhone) {
            customerMap.set(cleanPhone, { ...c, totalOrders: 0, successfulOrders: 0, returnedOrders: 0, totalSpent: 0 });
        }
    });
  ...
  }, [orders, persistentCustomers, loyaltyData]);
  ...
  const handleSavePoints = (phone: string, points: number) => {
    ...
    setCustomers(prev => prev.map(c => c.phone === phone ? { ...c, loyaltyPoints: points } : c));
