...
const CheckoutPage: React.FC<CheckoutPageProps> = ({ settings, cart, onPlaceOrder }) => {
  ...
  useEffect(() => {
      if(shippingCompany) {
          ...
          if (defaultArea) {
              ...
              setGovernorate(defaultArea);
          }
      }
  }, [shippingCompany, settings.shippingOptions]);

  const shippingOptions = useMemo(() => {
      const options = settings.shippingOptions[shippingCompany] || [];
  ...
  }, [settings.shippingOptions, shippingCompany]);

  const { subtotal, shippingFee, total } = useMemo(() => {
    const sub = cart.reduce((acc, item) => acc + item.price * item.quantity, 0);
    const selectedOption = shippingOptions.find(opt => opt.label === (governorate || shippingArea));
    
    let fee = selectedOption ? selectedOption.price : 0;
    if (selectedOption && city) {
        const cityOpt = selectedOption.cities?.find(c => c.name === city);
  ...
  }, [cart, shippingArea, governorate, city, shippingOptions, discountAmount]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
        const orderId = onPlaceOrder({
            ...
            customerAddress,
            shippingCompany,
            shippingArea: governorate || shippingArea,
            governorate: governorate || shippingArea,
            city,
            shippingFee,
            notes: finalNotes,
            redeemedPoints: 0,
                ...
                <h2 className="text-xl font-bold flex items-center gap-2"><Truck size={20}/> بيانات الشحن</h2>
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                     ...
                     <div className="grid grid-cols-2 gap-2">
                         <select 
                            required 
                            value={governorate || shippingArea} 
                            onChange={e => {
                                setGovernorate(e.target.value);
                                setShippingArea(e.target.value);
                                setCity('');
                            }} 
                            className="w-full p-3 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                         >
                         ...
                         <select 
                            ...
                            className="w-full p-3 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                            disabled={!(governorate || shippingArea)}
                         >
                            <option value="" disabled>المدينة</option>
                            {(shippingOptions.find(o => o.label === (governorate || shippingArea))?.cities || []).map(c => (
