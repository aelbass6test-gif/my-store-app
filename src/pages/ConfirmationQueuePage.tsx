...
const ConfirmationQueuePage: React.FC<ConfirmationQueuePageProps> = ({ orders, setOrders, currentUser, settings, activeStore }) => {
    ...
    useEffect(() => {
        if (activeOrder) {
            ...
            setEditedGovernorate(activeOrder.governorate || activeOrder.shippingArea || '');
    ...
    const handleSaveAddress = () => {
        ...
        const targetGov = editedGovernorate || activeOrder.shippingArea;
        
        if (targetGov) {
             const shippingOptions = settings.shippingOptions[activeOrder.shippingCompany] || [];
             const selectedOption = shippingOptions.find(opt => opt.label === targetGov);
             if (selectedOption) {
                 newShippingFee = selectedOption.price;
                 // Check for city-specific price
                 if (editedCity) {
                     const cityOpt = selectedOption.cities?.find(c => c.name === editedCity);
                     if (cityOpt && cityOpt.shippingPrice !== undefined && cityOpt.shippingPrice !== null) {
                         newShippingFee = cityOpt.shippingPrice;
                     }
                 }
             }
        }
        ...
        setOrders(currentOrders => 
            currentOrders.map(o => o.id === activeOrder.id ? { 
                ...o, 
                customerAddress: editedAddress, 
                shippingArea: targetGov, 
    ...
    const activeShippingOptions = useMemo(() => {
        if (!activeOrder) return [];
        const options = settings.shippingOptions[activeOrder.shippingCompany] || [];
                                            ...
                                            {isEditingAddress ? (
                                                <div className="space-y-2">
                                                    <div className="grid grid-cols-2 gap-2">
                                                        ...
                                                        <select 
                                                            ...
                                                            className="w-full p-2 bg-slate-100 dark:bg-slate-700 rounded-md text-sm font-bold"
                                                            disabled={!editedGovernorate}
                                                        >
                                                            <option value="">المدينة...</option>
                                                            {(activeShippingOptions.find(o => o.label === editedGovernorate)?.cities || []).map(c => (
                                                ...
                                                <div className="flex items-start justify-between">
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-slate-500 mt-1">{activeOrder.governorate || activeOrder.shippingArea}{activeOrder.city ? ` - ${activeOrder.city}` : ''}</p>
                                                    </div>
                                                    <button onClick={() => { setIsEditingAddress(true); setEditedAddress(activeOrder.customerAddress); setEditedGovernorate(activeOrder.governorate || activeOrder.shippingArea); setEditedCity(activeOrder.city || ''); }} className="p-1 text-slate-400 hover:text-blue-500"><Edit3 size={14}/></button>
                                                </div>
                                            )}
                                        </div>
                                    </DetailSection>
