...
-- 19. Webhook Settings (For third-party platform integrations)
create table if not exists public.webhook_settings (
  id bigint generated by default as identity primary key,
  store_id text not null references public.stores_data(id) on delete cascade,
  platform text not null, -- e.g., 'wuilt', 'salla', 'shopify'
  secret_key text, -- Used to verify incoming webhook signatures
  whatsapp_api_url text, -- For sending WhatsApp alerts on errors
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  unique(store_id, platform)
);

-- 20. Webhook Logs (Activity history of incoming webhooks)
create table if not exists public.webhook_logs (
  id bigint generated by default as identity primary key,
  store_id text references public.stores_data(id) on delete cascade,
  platform text,
  payload jsonb,
...
drop policy if exists "Public Access Webhook Settings" on public.webhook_settings;
drop policy if exists "Public Access Webhook Logs" on public.webhook_logs;
...
create policy "Public Access Webhook Settings" on public.webhook_settings for all using (true) with check (true);
